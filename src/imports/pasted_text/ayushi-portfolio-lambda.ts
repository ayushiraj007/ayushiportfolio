 ...
 * AWS Lambda Function URL, Node 24.x
 *
 * Handles MCP JSON-RPC directly without StreamableHTTPServerTransport,
 * avoiding Node req/res stream shimming issues in Lambda.
 *
 * Tools exposed:
 *   chat         – conversational interface in Ayushi's voice
 *   get_resume   – structured resume JSON
 *   get_projects – project highlights
 *   get_skills   – skill categories
 */
...
const TOOLS = [
  {
    name: "chat",
    description: "Talk to Ayushi's portfolio assistant. Speaks in Ayushi's own voice.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "get_resume",
    description: "Returns Ayushi's structured resume as JSON.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_projects",
    description: "Returns Ayushi's key projects with impact metrics and tools.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_skills",
...
async function executeTool(name, args) {
  if (name === "chat") {
    const text = await callLLM(args.message ?? "", args.history ?? []);
  ...
  try {
    ...
    if (method === "initialize") {
      return ok({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "ayushi-portfolio", version: "1.0.0" },
      });
    }
